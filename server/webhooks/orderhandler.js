  ...
  try {
    ...
    for (const item of orderData.line_items) {
      ...
      if (existingGroup.length === 0) {
        ...
        console.log(`Quantity increased for ${itemId}: ${totalExistingQty} -> ${item.quantity} (diff: ${diff})`);
        ...
        console.log(`Quantity decreased for ${itemId}: ${totalExistingQty} -> ${item.quantity}`);
        ...
        for (const existingItem of existingGroup) {
          ...
          if (existingItem.quantity <= remaining) {
            ...
            console.log(`Deleted line_item ${existingItem.id} (qty: ${existingItem.quantity})`);
          } else {
            // 减少这个条目的数量
            ...
            console.log(`Updated line_item ${existingItem.id}: ${existingItem.quantity} -> ${newQty}`);
            remaining = 0;
          }
        }
      }
      // 如果 quantity 相等，不做任何操作
    }

    // 删除被移除的 items（所有相关条目）
    itemGroups.forEach((group, baseId) => {
      if (!currentItemIds.has(baseId)) {
        console.log(`Item ${baseId} removed from order`);
        group.forEach(item => {
          // 删除 line_item
          ...
          console.log(`Deleted line_item ${item.id}, kept waiting transfer items`);
        });
      }
    });
