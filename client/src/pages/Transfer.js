...
const Transfer = () => {
  ...
  const handleTransferSubmit = async () => {
    ...
    // 🆕 前置校验 transfer quantity，避免无效值还要跑一趟后端才报错
    const qtyCheck = parseInt(transferData.transferQuantity);
    if (isNaN(qtyCheck) || qtyCheck < 1 || qtyCheck > transferModal.quantity) {
      alert(`Transfer quantity must be between 1 and ${transferModal.quantity}`);
      return;
    }
    ...
    try {
      ...
      showToast(`Error updating transfer: ${detail}`);
    }
  };
